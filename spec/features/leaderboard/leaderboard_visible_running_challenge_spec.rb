...
feature "leaderboard invisible challenge running" do
  ...
  scenario 'public user' do
    ...
    expect(page).not_to have_content('Leaderboard is not displayed yet')
  ...
  scenario 'organizer', js: true do
    ...
    visit "/challenges/#{challenge.slug}/leaderboards"
    expect(page).not_to have_content('Leaderboard is only visible to organizers')
    expect(page).to have_content('99')
    expect(page).to have_content('98')
