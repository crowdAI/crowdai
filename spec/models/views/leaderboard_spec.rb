...
describe Leaderboard do
  ...
  context '1 round challenge' do
    ...
    describe 'order by primary asc, secondary asc' do
      ...
      it { expect(Leaderboard.first.participant).to eq(participant1) }
      it { expect(Leaderboard.first.row_num).to eq(1) }
      it { expect(Leaderboard.first.score).to eq(30.05) }
      it { expect(Leaderboard.first.score_secondary).to eq(0.0001) }
      it { expect(Leaderboard.second.participant).to eq(participant2) }
      it { expect(Leaderboard.second.row_num).to eq(2) }
      it { expect(Leaderboard.second.score).to eq(31.00) }
      it { expect(Leaderboard.second.score_secondary).to eq(0.0003) }
    end
    ...
    describe 'order by primary desc, secondary asc' do
      ...
      it { expect(Leaderboard.count).to eq(2) }
      it { expect(Leaderboard.first.participant).to eq(participant2) }
      it { expect(Leaderboard.first.row_num).to eq(1) }
      it { expect(Leaderboard.first.score).to eq(36.00) }
      it { expect(Leaderboard.first.score_secondary).to eq(0.0003) }
      it { expect(Leaderboard.second.participant).to eq(participant1) }
      it { expect(Leaderboard.second.row_num).to eq(2) }
      it { expect(Leaderboard.second.score).to eq(32.05) }
      it { expect(Leaderboard.second.score_secondary).to eq(0.0003) }
    end
    ...
    describe 'order by primary asc, secondary desc' do
      ...
      it { expect(Leaderboard.first.row_num).to eq(1) }
      it { expect(Leaderboard.first.score).to eq(30.05) }
      it { expect(Leaderboard.first.score_secondary).to eq(0.0002) }
      it { expect(Leaderboard.second.participant).to eq(participant2) }
      it { expect(Leaderboard.second.row_num).to eq(2) }
      it { expect(Leaderboard.second.score).to eq(31.00) }
      it { expect(Leaderboard.second.score_secondary).to eq(0.0003) }
